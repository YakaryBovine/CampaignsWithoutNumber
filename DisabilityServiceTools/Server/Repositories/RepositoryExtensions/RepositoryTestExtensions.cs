...
namespace DisabilityServiceTools.Server.Repositories.RepositoryExtensions
{
  ...
  {
    public static IQueryable<Test> Search(this IQueryable<Test> products, string searchTearm)
    {
    ...
    public static IQueryable<Test> Sort(this IQueryable<Test> products, string orderByQueryString)
    {
      if (string.IsNullOrWhiteSpace(orderByQueryString))
        return products.OrderBy(e => e.Id.ToString());

      var orderParams = orderByQueryString.Trim().Split(',');
      var propertyInfos = typeof(Test).GetProperties(BindingFlags.Public | BindingFlags.Instance);
